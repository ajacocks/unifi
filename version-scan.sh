...
SPECFILE=~/rpmbuild/SPECS/unifi.spec

DLURL=$( egrep '^Source0:\s+' $SPECFILE | sed 's/^Source0://' )
SOURCEFILE=~/rpmbuild/SOURCES/$( echo $DLURL | sed 's/http.*\///' )
...
if [ ! -f $SOURCEFILE ]; then
  echo "Please please the required sourcefiles in ~/rpmbuild/SOURCES. Exiting!"
  exit 3
fi
